"""represents a player in the game. AI inherits from this class"""
...
class player:
    ...
    def build_road(self, v1, v2, board):
        ...
        if (
            self.resources["BRICK"] > 0 and self.resources["WOOD"] > 0
            ...
            if self.roadsLeft > 0:  # Check if player has roads left
                ...
                print(
                    "{} Built a Road. MaxRoadLength: {}".format(
    ...
    def build_settlement(self, vCoord, board):
        ...
        if (
            ...
            if self.settlementsLeft > 0:  # Check if player has settlements left
                ...
                board.updateBoardGraph_settlement(
                    vCoord, self
                )  # update the overall boardGraph
    ...
    def build_city(self, vCoord, board):
        ...
        if (
            self.resources["WHEAT"] >= 2 and self.resources["ORE"] >= 3
        ):  # Check if player has resources available
            if self.citiesLeft > 0:
                self.buildGraph["CITIES"].append(vCoord)
                self.settlementsLeft += (
                    1  # Increase number of settlements and decrease number of cities
                )
                self.citiesLeft -= 1
    ...
    def get_road_length(self, board) -> int:
        """Function to calculate road length for longest road calculation
        Use both player buildgraph and board graph to compute recursively
    ...
    def check_path_length(
        self, edge, edgeList, roadLength, vertexList, boardGraph
    ) -> None:
        """Function to checl the path length from a current edge to all possible other vertices not yet visited by it"""
        ...
        # if no neighboring roads exist append the road Length up to this edge
    ...
    def get_neighboring_roads(
        self, road_i, boardGraph, visitedRoads, visitedVertices
    ) -> list:
        ...
        for edge in self.buildGraph["ROADS"]:
            ...
            if edge not in visitedRoads:  # If it is a new distinct edge
                if boardGraph[v2].state["Player"] in [
                    self,
                    None,
                ]:  # Add condition for vertex to be not colonised by anyone else
                    if (
                        edge[0] == v2 and edge[0] not in visitedVertices
                    ...
                    if (
                        edge[1] == v2 and edge[1] not in visitedVertices
    ...
    def draw_devCard(self, board):
        ...
        if (
            ...
            print(
                "{} drew a {} from Development Card Stack".format(self.name, cardDrawn)
            )
    ...
    def play_devCard(self, game):
        ...
        if devCardPlayed == "YEAROFPLENTY":
            ...
            r1, r2 = "", ""
            while (r1 not in self.resources.keys()) or (
                r2 not in self.resources.keys()
            ):
        ...
        if devCardPlayed == "MONOPOLY":
            ...
            while resourceToMonopolize not in self.resources.keys():
                resourceToMonopolize = input(
                    "Enter resource name to monopolise: "
                ).upper()
    ...
    def trade_with_bank(self, r1, r2):
        ...
        r1_port = "2:1 " + r1
        if (
            r1_port in self.portList and self.resources[r1] >= 2
    ...
    # Function to initiate a trade - with bank or other players
        ...
        if trade_type == "BANK":
            ...
            while resourceToTrade not in self.resources.keys():
                resourceToTrade = input(
                    "Enter resource name to trade with bank:"
                ).upper()

            # Player to select resource to receive - disallow receiving same resource as traded
            resourceToReceive = ""
            while (resourceToReceive not in self.resources.keys()) or (
                resourceToReceive == resourceToTrade
            ):
                resourceToReceive = input(
                    "Enter resource name to receive from bank:"
                ).upper()
            ...
            playerToTrade_name = ""
            while (playerToTrade_name not in playerNames) or (
                playerToTrade_name == self.name
            ):
            ...
            while resourceToTrade not in self.resources.keys():
                resourceToTrade = input(
                    "Enter resource name to trade with player {}:".format(
                        playerToTrade_name
                    )
                ).upper()
                # Reset if invalid resource is chosen
                if (
                    resourceToTrade in self.resources.keys()
                    and self.resources[resourceToTrade] == 0
                ):
                    resourceToTrade = ""
            ...
            while (resource_traded_amount > self.resources[resourceToTrade]) or (
                resource_traded_amount < 1
            ):
                resource_traded_amount = int(
                    input(
                        "Enter quantity of {} to trade with player {}:".format(
            ...
            while (resourceToReceive not in self.resources.keys()) or (
                ...
                resourceToReceive = input(
                    "Enter resource name to receive from player {}:".format(
                        playerToTrade_name
                    )
                ).upper()
                # Reset if invalid resource is chosen
                if (
                    resourceToReceive in self.resources.keys()
                    and playerToTrade.resources[resourceToReceive] == 0
                ):
                    resourceToReceive = -""
                    print(
                        "Player {} doesn't have any {} to trade".format(
    ...
    def discardResources(self):
        ...
        if totalResourceCount > maxCards:
            ...
            for i in range(numCardsToDiscard):
                ...
                resourceToDiscard = ""
                while (resourceToDiscard not in self.resources.keys()) or (
                    self.resources[resourceToDiscard] == 0
                ):
                ...
                print(
                    "Player {} discarded a {}, and needs to discard {} more cards".format(
