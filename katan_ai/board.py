...
class catanBoard(hexTile, Vertex):
    ...
    def __init__(self) -> None:
        ...
        self.boardGraph = (
            {}
        )  # Dict to store the vertex objects with the pixelCoordinates as keys

        self.edgeLength = 80  # Specify for hex size
        self.size = self.width, self.height = 1000, 800
        self.flat = Layout(
            layout_flat,
            Point(self.edgeLength, self.edgeLength),
    ...
    def getHexCoords(self, hexInd: int) -> Axial_Point:
    ...
    def getRandomResourceList(self) -> list:
    ...
    def checkHexNeighbors(self, randomIndices) -> bool:
        ...
        for pos, random_Index in enumerate(randomIndices):
            ...
            for neighbor_index in hexNeighborIndexList[pos]:
                rollValueOnNeighbor = self.resourcesList[
                    randomIndices[neighbor_index]
                ].num
    ...
    def generateVertexGraph(self) -> None:
        for hexTile in self.hexTileDict.values():
            hexTileCorners = polygon_corners(
                self.flat, hexTile.hex
            )  # Get vertices of each hex
    ...
    def updateVertexGraph(self, vertexCoordList, hexIndx):
        for v in vertexCoordList:
            ...
            if v in self.vertex_index_to_pixel_dict.values():
                ...
                self.vertex_index_to_pixel_dict[
                    self.vertexIndexCount
                ] = v  # Create the index-pixel key value pair
    ...
    def printGraph(self):
        ...
        for node in self.boardGraph.keys():
            print(
                "Pixel:{}, Index:{}, NeighborVertexCount:{}, AdjacentHexes:{}".format(
                    node,
                    self.boardGraph[node].vertexIndex,
                    len(self.boardGraph[node].edgeList),
                    self.boardGraph[node].adjacentHexList,
                )
            )
    ...
    def updatePorts(self):
        ...
        for portType, portVertexPair_count in port_dict.items():
            ...
            for i in range(portVertexPair_count):  # Number of ports to assign
                portVertices += port_pair_list[
                    randomPortIndices[randomPortIndex_counter]
                ]  # Add randomized port
        ...
        for portType, portVertexIndex_list in port_dict.items():
            for v_index in portVertexIndex_list:  # Each vertex
                vertexPixel = self.vertex_index_to_pixel_dict[
                    v_index
                ...
                for indx, v_i in enumerate(self.boardGraph[vertex_i].edgeList):
                    if (self.boardGraph[vertex_i].edgeState[indx][1] == False) and (
                        ...
                        if (v_i, vertex_i) not in colonisableRoads.keys() and (
                            vertex_i,
                            v_i,
                        ) not in colonisableRoads.keys():  # If the edge isn't already there in both its regular + opposite orientation
        ...
        for existingRoad in player.buildGraph["ROADS"]:
            for vertex_i in existingRoad:  # Iterate over both vertices of this road
                ...
                if vertex_i not in colonisableVertices.keys():
                    if self.boardGraph[
                        vertex_i
                    ].isColonised:  # Check if this vertex is already colonised
                        break

                    canColonise = True
                    for v_neighbor in self.boardGraph[
                        vertex_i
                    ].edgeList:  # Check each of the neighbors from this vertex
    ...
    def get_setup_settlements(self, player):
        ...
        for vertexCoord in self.boardGraph.keys():
            ...
            for v_neighbor in potentialVertex.edgeList:
                if self.boardGraph[
                    v_neighbor
                ].isColonised:  # Check if any of first neighbors are colonised
                    canColonise = False
                    break

            if (
                canColonise
            ):  # If the vertex is colonisable add it to the dict with its Rect
