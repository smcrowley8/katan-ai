        ...
        if diceRoll != 7:  # Collect resources if not a 7
            ...
            for player_i in list(self.playerQueue.queue):
                ...
                for settlementCoord in player_i.buildGraph["SETTLEMENTS"]:
                    for adjacentHex in self.board.boardGraph[
                        settlementCoord
                    ].adjacentHexList:  # check each adjacent hex to a settlement
                        if (
                            adjacentHex in hexResourcesRolled
                            and self.board.hexTileDict[adjacentHex].robber == False
                        ):  # This player gets a resource if hex is adjacent and no robber
                            resourceGenerated = self.board.hexTileDict[
                                adjacentHex
                            ].resource.type
                            player_i.resources[resourceGenerated] += 1
                            print(
    ...
    def check_longest_road(self, player_i):
        if player_i.maxRoadLength >= 5:  # Only eligible if road length is at least 5
            ...
            for p in list(self.playerQueue.queue):
                if (
                    p.maxRoadLength >= player_i.maxRoadLength and p != player_i
                ):  # Check if any other players have a longer road
                    longestRoad = False

            if (
                longestRoad and player_i.longestRoadFlag == False
                ...
                print(
                    "Player {} takes Longest Road {}".format(player_i.name, prevPlayer)
                )

    # function to check if a player has the largest army - after playing latest knight
    def check_largest_army(self, player_i):
        if (
            player_i.knightsPlayed >= 3
        ):  # Only eligible if at least 3 knights are player
            largestArmy = True
            for p in list(self.playerQueue.queue):
                if (
                    p.knightsPlayed >= player_i.knightsPlayed and p != player_i
                ):  # Check if any other players have more knights played
                    largestArmy = False

            if (
                largestArmy and player_i.largestArmyFlag == False
                ...
                print(
                    "Player {} takes Largest Army {}".format(player_i.name, prevPlayer)
                )
            ...
            for currPlayer in self.playerQueue.queue:
                ...
                while turnOver == False:
                    ...
                    if currPlayer.isAI:
                        ...
                        for (
                            e
                        ) in pygame.event.get():  # Get player actions/in-game events
                            # print(e)
                            ...
                            if e.type == pygame.MOUSEBUTTONDOWN:
                                ...
                                if self.boardView.buildRoad_button.collidepoint(e.pos):
                                    ...
                                    if (
                                        diceRolled == True
                                    ):  # Can only build after rolling dice
                                ...
                                if self.boardView.buildSettlement_button.collidepoint(
                                    e.pos
                                ):
                                    if (
                                        diceRolled == True
                                ...
                                if self.boardView.buildCity_button.collidepoint(e.pos):
                                    if (
                                        diceRolled == True
                                    ):  # Can only build city after rolling dice
                                ...
                                if self.boardView.tradePlayers_button.collidepoint(
                                    e.pos
                                ):
                                ...
                                if self.boardView.endTurn_button.collidepoint(e.pos):
                                    if (
                                        diceRolled == True
                                    ):  # Can only end turn after rolling dice
                                        print("Ending Turn!")
                                        turnOver = (
                                            True  # Update flag to nextplayer turn
                                        )
