...
class catanGameView:
    ...
    def displayInitialBoard(self):
        ...
        pygame.draw.rect(
            self.screen,
            pygame.Color("royalblue2"),
            (0, 0, self.board.width, self.board.height),
        )  # blue background
        ...
        for hexTile in self.board.hexTileDict.values():
            ...
            pygame.draw.polygon(
                self.screen,
                pygame.Color(
                    hexTileColor_rgb[0], hexTileColor_rgb[1], hexTileColor_rgb[2]
                ),
                hexTileCorners,
                self.board.width == 0,
            )
            ...
            hexTile.pixelCenter = hex_to_pixel(
                self.board.flat, hexTile.hex
            ...
            if hexTile.resource.type != "DESERT":  # skip desert text/number
                resourceText = self.font_resource.render(
                    str(hexTile.resource.type) + " (" + str(hexTile.resource.num) + ")",
                    False,
                    (0, 0, 0),
                )
                self.screen.blit(
    ...
    def draw_road(self, edgeToDraw, roadColor):
        pygame.draw.line(
            self.screen, pygame.Color(roadColor), edgeToDraw[0], edgeToDraw[1], 10
        )
    ...
    def draw_possible_settlement(self, vertexToDraw, color):
        possibleSettlement = pygame.draw.circle(
            self.screen,
            pygame.Color(color),
            (int(vertexToDraw.x), int(vertexToDraw.y)),
            20,
            3,
        )
    ...
    def draw_possible_players_to_rob(self, vertexCoord):
        possiblePlayer = pygame.draw.circle(
            self.screen,
            pygame.Color("black"),
            (int(vertexCoord.x), int(vertexCoord.y)),
            35,
            5,
        )
    ...
    def displayGameButtons(self):
        ...
        pygame.draw.rect(
            self.screen, pygame.Color("gray33"), self.buildSettlement_button
        )
    ...
    def displayRobber(self):
        ...
        self.screen.blit(
            robberText, (int(robberCoords.x) - 20, int(robberCoords.y) - 35)
        )
    ...
    def displayGameScreen(self):
        ...
        for player_i in list(
            self.game.playerQueue.queue
    ...
    def buildSettlement_display(self, currentPlayer, verticesPossibleDict):
        ...
        while mouseClicked == False:
            if (
                self.game.gameSetup
            ):  # during gameSetup phase only exit if settlement is built
                for e in pygame.event.get():
                    if e.type == pygame.QUIT:
                        sys.exit(0)
                    if e.type == pygame.MOUSEBUTTONDOWN:
