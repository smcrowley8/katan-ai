...
class hexTile:
    ...
    def __init__(
        self, hexIndex: int, hexResource, axialCoords: Axial_Point, neighborList=None
    ) -> None:
        """

        Params:
            hexIndex: int: number 0-N where N is the number of total playable tiles. Unplayable tiles have index=-1
            hexResource:
            axaialCoords:
        """
    ...
    def displayHexInfo(self) -> None:
        print(
            "Index:{}; Hex:{}; Axial Coord:{}".format(
                self.index, self.resource, self.coord
            )
        )
        return None

    def displayHexNeighbors(self) -> None:
        """display info of neighbors"""
...
class Vertex:
    ...
    def __init__(self, pixelCoord, adjHexIndex, vIndex):
        ...
        self.adjacentHexList = [
            adjHexIndex
        ]  # List to store indices of 3 adjacent hexes
        self.edgeState = [
            [None, False],
            [None, False],
            [None, False],
        ]  # Nested list to determine if a road is built on edge, and player building road
    ...
    def isAdjacent(self, v1, v2):
        dist = (
            (v1.pixelCoordinates.x - v2.pixelCoordinates.x) ** 2
            + (v1.pixelCoordinates.y - v2.pixelCoordinates.y) ** 2
        ) ** 0.5
